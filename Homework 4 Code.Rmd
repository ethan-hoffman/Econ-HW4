...
ggplot(data = damages, aes (x = warming, y = damages))+
  ...
  annotate(geom = "text", x = 3.5, y = 1000000000000000, label = "y = (-1.95 * 10^-13) - (3.07 * 10^-12)") +
  theme_minimal()
```
...
x <- damages_lm[["coefficents"]][["warming"]]
...
##Need to fix because of mutate error

###Difference in Damages over Time that Arise from the Pulse
ggplot(data = warming, aes(x = year,y = damage_difference))+
  geom_point()+
  theme_minimal()
```
...
ggplot(data = warming, aes(x = year,y = damage_annual_Cost))+
  geom_point()+
  theme_minimal()
```
